...
def _scan_generation_params(meta: dict, ev: ProvenanceEvidence) -> None:
    """Scan PNG text chunks from local SD tools (A1111 / ComfyUI / NovelAI).
...
def _scan_iptc_source_type(meta: dict, ev: ProvenanceEvidence) -> None:
    """Scan the full IPTC DigitalSourceType vocabulary, not just the AI value."""
    for key, value in meta.items():
...
def _c2pa_read(path: Path) -> tuple[bool, str | None, str | None, dict | None]:
    """Check for and read a C2PA manifest.

    Verify its existence and validity, and return the generator name if present.:
